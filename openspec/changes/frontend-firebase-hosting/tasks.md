...
- [x] **Pipeline automático confirmado end-to-end.** O merge do PR #15 (`7951df2`) disparou o trigger `helpdesk-deploy` sozinho — build `14b6417e` no Cloud Build, `SUCCESS`, iniciado 3s depois do merge, `COMMIT_SHA` batendo com o commit de merge. Deployou a revisão `helpdesk-backend-00038-27s` com `ALLOWED_ORIGIN`/`FRONTEND_URL` já corretos: o trigger não carrega substituições antigas sobrescrevendo os defaults novos do `cloudbuild.yaml`. Esta era a lacuna real da change — até aqui só deploys manuais via Cloud Shell tinham sido testados.
