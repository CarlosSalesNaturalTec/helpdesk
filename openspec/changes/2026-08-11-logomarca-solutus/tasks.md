...
- [ ] 5.4 Buildar a imagem Docker, subir o container e gerar um PDF confirmando que o ativo do backend está presente na etapa de runtime. **Bloqueado neste ambiente**: `docker build` falha com `429 Too Many Requests` ao puxar `node:20-alpine` do Docker Hub via o proxy da sandbox (não é um problema do Dockerfile). O `COPY` do `Dockerfile` e a resolução do caminho `LOGO_PRINT_PATH` a partir de `backend/dist/` já foram validados diretamente (ver commit) — falta apenas rodar `docker build` num ambiente com acesso ao registry.
