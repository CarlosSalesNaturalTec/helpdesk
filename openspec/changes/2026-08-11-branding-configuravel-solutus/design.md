...
- **Título do HTML via substituição do Vite.** O Vite substitui `%VITE_APP_NAME%` em `index.html` no build, sem *flash* de título no carregamento — preferível a definir `document.title` em runtime. Na prática, a substituição nativa do Vite não aplica fallback quando a env var não está definida (deixa o placeholder literal), o que quebraria a garantia de compatibilidade sem as novas variáveis; por isso `vite.config.ts` ganhou um pequeno plugin `transformIndexHtml` que aplica os mesmos fallbacks (`SOLUTUS` / `Instituto Setes`) antes da substituição nativa rodar.
