...
**RF09 — Notificações por E-mail:** O sistema deve enviar e-mail automaticamente para o Solicitante nos seguintes eventos: quando um Técnico assume seu chamado, quando o chamado muda para "Aguardando" (com a mensagem do Técnico), quando o chamado é resolvido, e quando um Gestor de TI ou Diretor realiza o fechamento administrativo de um chamado informando seu encerramento. O sistema também deve notificar o Técnico responsável (ou a fila de técnicos da unidade) quando um chamado fechado for reaberto, e o Técnico responsável deve ser notificado quando o Solicitante enviar uma mensagem em um chamado com status "Aguardando". Os e-mails devem conter o número do chamado e as informações relevantes da atualização.
