...
**RF09 — Notificações por E-mail:** O sistema deve enviar e-mail automaticamente para o Solicitante nos seguintes eventos: quando um Técnico assume seu chamado, quando o chamado muda para "Aguardando" (com a mensagem do Técnico), quando o chamado é resolvido, e quando um Gestor de TI ou Diretor realiza o fechamento administrativo de um chamado informando seu encerramento. O sistema também deve notificar o Técnico responsável (ou a fila de técnicos da unidade) quando um chamado fechado for reaberto, e o Técnico responsável deve ser notificado quando o Solicitante enviar uma mensagem em um chamado com status "Aguardando". Adicionalmente, quando um Gestor de TI ou Diretor reatribui um chamado, o novo Técnico responsável e o Solicitante devem ser notificados por e-mail. Os e-mails devem conter o número do chamado e as informações relevantes da atualização.
...
- Para Diretores e Gestores de TI (Relatório Local): apresenta os quatro cards-resumo numéricos (Total de Tickets, Taxa de Fechamento (%), Tempo Médio de Atendimento (horas) e Satisfação Média) e gráficos de distribuição restritos aos chamados de sua Unidade. O cálculo do TMA deve descontar automaticamente o tempo total em que o chamado permaneceu com status "Aguardando". Chamados encerrados via fechamento administrativo, sem registro de nota de satisfação, devem ser excluídos do cálculo da Satisfação Média.
- Para Administrador do Sistema (Relatório Geral/Comparativo): apresenta os mesmos cards-resumo consolidados em nível global, filtro de Unidade para visualização de dados de forma isolada, e um gráfico comparativo de volumetria e tipos de chamado entre as diversas unidades do Instituto. As mesmas regras de cálculo do TMA e da Satisfação Média aplicam-se a este nível consolidado.
